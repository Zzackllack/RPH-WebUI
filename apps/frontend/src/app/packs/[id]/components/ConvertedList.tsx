...
export default function ConvertedList({ packs, API }: { packs: ApiResourcePack[]; API: string }) {
  ...
  return (
    <div className="minecraft-card p-6 mt-8 space-y-2 border border-emerald-400/20 bg-gradient-to-br from-white/80 to-emerald-50/60 dark:from-gray-900/80 dark:to-emerald-900/40 shadow-md">
      <h2 className="text-xl font-semibold text-emerald-700 dark:text-emerald-300 mb-2">Converted Versions</h2>
      <ul className="space-y-1">
        {packs.map((p) => (
          <li key={p.id} className="flex items-center justify-between text-sm">
            <span className="font-mono text-gray-700 dark:text-gray-300">{p.targetVersion ?? ""}</span>
            <a
