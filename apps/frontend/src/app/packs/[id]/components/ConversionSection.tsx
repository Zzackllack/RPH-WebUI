...
export default function ConversionSection({
  ...
  return (
    ...
    >
        ...
        >
          {polling ? (
            <span className="flex items-center gap-2">
              <svg className="animate-spin w-4 h-4 text-emerald-500" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"/></svg>
              <span>Converting…</span>
            </span>
          ) : (
            <span className="flex items-center gap-2">
              <svg className="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M5 12h14M12 5l7 7-7 7"/></svg>
      ...
      {touched && !valid && (
        <p className="text-red-500 text-xs mt-1">Please enter a valid version in the format x.x or x.x.x</p>
      )}
          ...
          <div className="flex items-center gap-2">
            ...
            {job.status === "PENDING" || job.status === "IN_PROGRESS" ? (
              ...
              <span className="flex items-center gap-1 text-red-500">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2"/><path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M8 12h8"/></svg>
                <span>Failed</span>
              </span>
            ) : job.status === "COMPLETED" ? (
              <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2"/><path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4"/></svg>
                <span>Completed</span>
              </span>
            ) : (
              <span className="text-gray-500">{String(job.status).replace("_", " ")}</span>
            )}
          </div>
          {job.status === "FAILED" && (
