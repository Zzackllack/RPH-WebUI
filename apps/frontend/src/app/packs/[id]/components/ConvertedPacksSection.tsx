...
export default function ConvertedPacksSection({ packs }: { packs: ApiResourcePack[] }) {
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-lg truncate mb-1" title={pack.originalFilename}>
            ...
            <div className="space-y-3 mb-6 flex-1">
              <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>{pack.targetVersion ?? "Unknown version"}</span>
              </div>
              <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                <span>Pack Format: {pack.packFormat ?? "-"}</span>
              </div>
              <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                <span>Size: {(pack.size / 1024 / 1024).toFixed(2)} MB</span>
              </div>
              <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                <span>Uploaded: {new Date(pack.uploadDate).toLocaleDateString()}</span>
              </div>
            </div>
